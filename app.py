...
def api_reset_password_request():
    ...
    if PASSWORD_RESET_URL_BASE:
        if "{token}" in PASSWORD_RESET_URL_BASE:
            ...
            separator = "" if PASSWORD_RESET_URL_BASE.endswith(('/', '#')) else ("?token=" if "?" not in PASSWORD_RESET_URL_BASE else "&token=")
            reset_link = f"{PASSWORD_RESET_URL_BASE}{separator}{token}" if separator else f"{PASSWORD_RESET_URL_BASE}{token}"
